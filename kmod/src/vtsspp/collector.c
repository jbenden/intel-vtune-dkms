...
#define VTSS_STACK_DUMP(x,t,r,bp)     VTSS_STORE_STATE((x), vtss_stack_dump(VTSS_PT_FLUSH_MODE ? (x)->trnd_aux : (x)->trnd, &((x)->stk), (t), (r), (bp)), VTSS_ST_STKDUMP)
...
//#define VTSS_STACK_DUMP(x,t,r,bp)     VTSS_STORE_STATE((x), 0, VTSS_ST_STKDUMP)
...
int vtss_mmap_all(struct vtss_task_data*, struct task_struct*);
int vtss_kmap_all(struct vtss_task_data*);

#ifdef VTSS_RECOVERY_LOGIC
static VTSS_DEFINE_SPINLOCK(vtss_recovery_lock);
...
static inline int is_aux_transport_allowed(void)
{
    return (vtss_client_major_ver > 1 || (vtss_client_major_ver == 1 && vtss_client_minor_ver >= 1));
}

static inline int is_aux_transport_ring_buffer(void)
{
...
static void vtss_target_ctrl_wakeup(struct vtss_transport_data* trnd, struct vtss_transport_data* trnd_aux)
{
    if (trnd) {
        if (trnd_aux && trnd_aux != trnd) {
            ...
            TRACE("wakeup for %s", transport_path_aux);
...
static void vtss_target_exit_work(VTSS_WORK_STRUCT *work)
{
    ...
    if (atomic_read(&vtss_collector_state) == VTSS_COLLECTOR_STOPPED) { // It's OK to call this function in (un)initialization state
        ERROR("Exit work is called after collection was stopped");
    }
...
static void vtss_target_exec_attach_work(VTSS_WORK_STRUCT *work)
{
    int rc;
...
static void vtss_target_add_mmap_work(VTSS_WORK_STRUCT *work)
{
...
static void vtss_target_dtr(vtss_task_map_item_t* item, void* args)
{
    ...
    if (VTSS_IS_NOTIFIER(tskd)) {
        ...
        if (vtss_find_task_by_tid(tskd->tid) != NULL) { /* task exist */
            ...
            TRACE("unregistered notifier for tid=%d", tskd->tid);
        } else {
            ERROR("(%d:%d): Task does not exist", tskd->tid, tskd->pid);
        }
    }
...
int vtss_target_new_part1(pid_t tid, pid_t pid, pid_t ppid, vtss_task_map_item_t *item)
{
    ...
    if (task != NULL && !(task->state & TASK_DEAD)) { /* task exist */
        ...
        tskd->state |= VTSS_ST_NOTIFIER;
        TRACE("registered notifier for tid=%d", tskd->tid);
#endif
#ifdef VTSS_GET_TASK_STRUCT
        ...
        ERROR("(%d:%d:%d): %s: don't exist or not valid", tskd->tid, tskd->pid, tskd->ppid, tskd->filename);
        vtss_target_del(item);
        return 0;
    }
    ...
    DEBUG_COLLECTOR("end: (%d:%d): u=%d, n=%d, done='%s'",
        tskd->tid, tskd->pid, atomic_read(&item->usage), atomic_read(&vtss_target_count), tskd->filename);
    if (tskd->tid == tskd->pid) REPORT("Attached to '%s' (pid: %d)", tskd->filename, tskd->pid);
    return 0;
}

static void vtss_target_new_part2(VTSS_WORK_STRUCT *work)
{
    struct vtss_work* my_work = (struct vtss_work*)work;
...
static void vtss_sched_switch_to(vtss_task_map_item_t* item, struct task_struct* task, unsigned long ip);
int vtss_target_new(pid_t tid, pid_t pid, pid_t ppid, const char* filename, int fired_tid, int fired_order)
{
    int rc = 0;
    size_t size = 0;
    ...
    DEBUG_COLLECTOR("attaching to (%d:%d:%d): %s", tid, pid, ppid, filename);
    ...
    if (rc) {
        ERROR("(%d:%d): Unable to init stack", tid, pid);
        atomic_dec(&vtss_kernel_task_in_progress);
        return rc;
    }
    task = vtss_find_task_by_tid(tskd->tid);
...
{
    if (!VTSS_COLLECTOR_IS_READY) {
        DEBUG_COLLECTOR("Collector is not ready: state: %d, tid: %d", VTSS_COLLECTOR_STATE(), child ? TASK_TID(child) : -1);
        return;
    }
...
void vtss_target_exec_enter(struct task_struct *task, const char *filename, const char *config)
{
    ...
    if (!VTSS_COLLECTOR_IS_READY) {
        DEBUG_COLLECTOR("Collector is not ready: state: %d, tid: %d", VTSS_COLLECTOR_STATE(), task ? TASK_TID(task) : -1);
        return;
    }
    if (atomic_read(&vtss_transport_state) != 1) {
    ...
    if ((atomic_read(&vtss_transport_state) == 1) && VTSS_COLLECTOR_IS_READY_OR_INITING) {
        ...
        if (item != NULL) {
            ...
            if ((!VTSS_IS_COMPLETE(tskd)) && (!VTSS_IS_MMAP_INIT(tskd))) {
                ...
                if (tmp != NULL) {
#ifdef VTSS_USE_NMI
                    char* pname = VTSS_DPATH(file, tmp, 960);
#else
...
static void vtss_dump_stack(struct vtss_task_data* tskd, struct task_struct* task, struct pt_regs* regs, unsigned long bp, int clear)
{
    if (task != current) {
        return;
    }
...
static void vtss_sched_switch_from(vtss_task_map_item_t* item, struct task_struct* task, unsigned long bp, unsigned long ip)
{
    unsigned long flags;
...
static void vtss_sched_switch_to(vtss_task_map_item_t* item, struct task_struct* task, unsigned long ip)
{
    ...
    if (likely(VTSS_IN_NEWTASK(tskd))) {
        ...
        if (unlikely((VTSS_ALWAYS_STORE_CTX || (reqcfg.trace_cfg.trace_flags & VTSS_CFGTRACE_CTX)) &&
            cpu_tskd != NULL &&
            ...
            state == VTSS_COLLECTOR_RUNNING))
        {
            ...
            if (VTSS_ALWAYS_STORE_CTX || (reqcfg.trace_cfg.trace_flags & VTSS_CFGTRACE_CTX)) {
                ...
                VTSS_STORE_SWAPIN(tskd, cpu, (void*)ip, NOT_SAFE);
            } else {
                if (reqcfg.cpuevent_count_v1 !=0) VTSS_STORE_SAMPLE(tskd, tskd->cpu, (void*)ip, NOT_SAFE);
...
#endif
            {
                ...
                if (unlikely(!VTSS_NEED_STACK_SAVE(tskd))) {
                    if (reqcfg.trace_cfg.trace_flags & VTSS_CFGTRACE_CTX) {
                        vtss_dump_stack(tskd, task, NULL, 0, start_rec_id < tskd->start_rec_id);
                    }
                }
...
static void vtss_notifier_sched_out(struct preempt_notifier *notifier, struct task_struct *next)
{
    ...
    if (item != NULL) {
        ...
        VTSS_PROFILE(ctx, vtss_sched_switch_from(item, current, bp, 0));
        vtss_task_map_put_item(item);
    }
}
...
static void vtss_notifier_sched_in(struct preempt_notifier *notifier, int cpu)
{
    ...
    if (item != NULL) {
        void* bp;
        VTSS_GET_CURRENT_BP(bp);
        VTSS_PROFILE(ctx, vtss_sched_switch_to(item, current, _THIS_IP_));
        vtss_task_map_put_item(item);
    } else {
        vtss_profiling_pause();
    }
...
static void vtss_pmi_record(struct pt_regs* regs, vtss_task_map_item_t* item, int is_bts_overflowed)
{
    ...
    if (likely(VTSS_IN_CONTEXT(tskd))) {
        ...
        if (likely(VTSS_IS_CPUEVT(tskd))) {
            ...
            {
                vtss_dump_stack(tskd, current, regs, 0, start_rec_id < tskd->start_rec_id);
                if (likely(VTSS_NEED_STACK_SAVE(tskd) &&
                    (reqcfg.trace_cfg.trace_flags & VTSS_CFGTRACE_STACKS) &&
                    !vtss_transport_is_overflowing(tskd->trnd) &&
                    vtss_trylock_stack(&tskd->stk)))
...
static void vtss_transport_fini_work(VTSS_WORK_STRUCT *work)
{
...
static void vtss_collector_check_status(void *ctx)
{
    ...
    if (vtss_cpuevents_clr_pebs_ovf()) {
        if (ctx)(*((int*)ctx))++;
        vtss_pmi_enable();
    }
}
...
int vtss_collection_fini(void)
{
    ...
    while (atomic_read(&vtss_kernel_task_in_progress) != 0) {
        i++;
        if (i==1000) ERROR("Kernel task is not finished. Tasks in progress: %d", atomic_read(&vtss_kernel_task_in_progress));
    }
...
static void vtss_cmd_stop_work(VTSS_WORK_STRUCT *work)
{
    vtss_cmd_stop();
    vtss_kfree(work);
}
...
int vtss_init(void)
{
    ...
    if (xen_initial_domain()) {
        ERROR("XEN dom0 is not supported");
        return -1;
    }
#endif

#ifdef VTSS_GET_TASK_STRUCT
    if (vtss_put_task_struct_init()) {
        return -1;
    }
#endif
