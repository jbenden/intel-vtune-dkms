...
static void vtss_create_memchunk_work(VTSS_WORK_STRUCT *work)
{
...
struct vtss_memchunk* vtss_create_memchunk_async(gfp_t flags, unsigned int size)
{
    ...
    if (in_atomic())
    {
        ...
        {
            DEBUG_MP("cannot create memchunk");
            atomic_dec(&g_vtss_kernel_task_in_progress);
        }
...
static void vtss_destroy_memchunk_work(VTSS_WORK_STRUCT *work)
{
    struct vtss_work* my_work = (struct vtss_work*)work;
    struct vtss_memchunk* chunk = NULL;
    if (!my_work)
...
static size_t vtss_memory_pool_total_size(void)
{
    struct list_head* p = NULL;
    struct list_head* tmp = NULL;
    size_t total = 0;
    ...
    {
        struct vtss_memchunk* chunk = list_entry(p, struct vtss_memchunk, list);
        if (chunk != NULL) total += chunk->size;
    }
    return total;
}
...
int vtss_memory_pool_init(void)
{
    ...
    REPORT("Reserved %ldMB for internal memory pool", vtss_memory_pool_total_size()/1024/1024);
