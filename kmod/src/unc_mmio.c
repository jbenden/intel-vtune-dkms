...
/* ------------------------------------------------------------------------- */
/*!
 * @fn       void unc_mmio_single_bar_Trigger_Read(id)
 *
...
/* ------------------------------------------------------------------------- */
/*!
 * @fn       void unc_mmio_multiple_bar_Trigger_Read(id)
 *
 * @param    id       Device index
 *
 * @return   None     No return needed
...
)
{
    ...
    if (dev_node) {
        ...
        SEP_DRV_LOG_ERROR_TRACE_OUT("PCB node not available in group %u!", cur_grp);
    }
    if (entries == 0) {
        use_default_busno = 1;
